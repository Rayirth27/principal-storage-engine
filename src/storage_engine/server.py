...
logging.basicConfig(
    ...
    format="%(asctime)s [%(levelname)s] %(message)s", #Inserting "Current Time" with the "LEVEL" with "message"
    datefmt="%Y-%m-%d %H:%M:%S",
)
logger = logging.getLogger("StorageServer") #This is a factory function. It asks Python’s logging system to check for a tracker with "StorageServer" name
...
# ensures our system telemetry is modular, identifiable, and ready to scale as we add more files to our database engine!
#If you only use the root logging.info(), it's all-or-nothing—you have to turn the volume up or down for the entire application at once.

def handle_client_connection(client_socket: socket.socket, client_address: tuple, engine: StorageEngine) -> None:
    """
    ...
    try:
        ...
        if data:
            ...
            logger.info(f"Received raw payload from client: '%s'", decoded_message)
...
def run_server(host: str = "127.0.0.1", port: int = 9999) -> None:
    ...
    engine = StorageEngine() # Reloaded old data from dump.json - ensuring data integrity as this check happens before opening ports
    #we opened the network ports first, a super-fast client could ask for a key before the disk had finished reading, receiving a false NIL error.
    ...
    signal.signal(signal.SIGINT, shutdown_handler)     # Using this we are not replying on Python's KetboardInterrupt - Triggered by Ctrl+C
    signal.signal(signal.SIGTERM, shutdown_handler)     # Triggered by OS

    server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)  # Here we tell the OS that we want to you TCP 
    server_socket.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR,1) # Using the SO_REUSEADDR - we use the port 9999 immediately whenever I want

    
    server_socket.bind((host, port)) # : If socket() was building the house, bind() is putting the house on a specific street address.
    ...
    logger.info("Thread pool initialized with strict ceiling of %d concurrent workers.", MAX_WORKERS)
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool: # manager
        try:
            while True: # server running forever until it is stopped.
                ...
                client_socket, client_address = server_socket.accept()              #The server execution stops right here.
                print(f"DEBUG: Server accepted connection from {client_address}")
                #Instantly hand the connection off to a background worker
                pool.submit(handle_client_connection, client_socket, client_address, engine) # Multi-threaded: 
